...
//-------------------- HISTOGRAM CLASS --------------------
Segment::Histogram::Histogram(int numDimensions, int numBinsPerDimension)
{
...
void Segment::Histogram::extractForegroundHistogram(std::vector<cv::Mat> & imgChannels, cv::Mat weights, bool useMatWeights, int x1, int y1, int x2, int y2)
{
...
void Segment::Histogram::extractBackGroundHistogram(std::vector<cv::Mat> & imgChannels, int x1, int y1, int x2, int y2, int outer_x1, int outer_y1, int outer_x2, int outer_y2)
{
    //extract pixel values and compute histogram
    double rangePerBinInverse = static_cast<double>(m_numBinsPerDim)/256.0;  // 1 / ( imgRange/numBinsPerDim )
    double sum = 0;
    for (int y = outer_y1; y < outer_y2+1; ++y){

        std::vector<const uchar *> dataPtr(m_numDim);
        for (int dim = 0; dim < m_numDim; ++dim)
            dataPtr[dim] = imgChannels[dim].ptr<uchar>(y);

        for (int x = outer_x1; x < outer_x2+1; ++x){
...
cv::Mat Segment::Histogram::backProject(std::vector<cv::Mat> & imgChannels)
{
    //just for code clarity
    cv::Mat & img = imgChannels[0];
...
//-------------------- SEGMENT CLASS --------------------
std::pair<cv::Mat, cv::Mat> Segment::computePosteriors(std::vector<cv::Mat> &imgChannels, int x1, int y1, int x2, int y2, cv::Mat weights, cv::Mat fgPrior, cv::Mat bgPrior, const Histogram &fgHistPrior, int numBinsPerChannel)
{
    ...
    int outer_y2 = std::min(imgChannels[0].rows-1, (int)(y2+offsetY+1));
    int outer_x1 = std::max(0, (int)(x1-offsetX));
    int outer_x2 = std::min(imgChannels[0].cols-1, (int)(x2+offsetX+1));

    //extract histogram from original data -> more pixels better representation of distr. by histograms
    Histogram hist_target = (fgHistPrior.m_numBinsPerDim == numBinsPerChannel && (size_t)fgHistPrior.m_numDim == imgChannels.size()) ? fgHistPrior : Histogram(imgChannels.size(), numBinsPerChannel);
    ...
    hist_background.extractBackGroundHistogram(imgChannels, x1, y1, x2, y2, outer_x1, outer_y1, outer_x2, outer_y2);
    ...
    double p_b = std::sqrt( (std::pow(outer_x2-outer_x1, 2) + std::pow(outer_y2-outer_y1, 2)) / (std::pow(x2-x1, 2) + std::pow(y2-y1, 2))) ;
    double p_o = 1./(p_b + 1);
